    ...
    fix: {
        ...
        func: (allLines, variables) => {
            ...
            if (range.start.line === range.end.line) {
                allLines[lineNumber] = allLines[lineNumber].replace(/"/g, "'");
            }
            // Multiline replacement: replace """ by '''
            else {
                allLines[range.start.line - 1] = allLines[range.start.line - 1].replace(/"""/g, "'''");
                allLines[range.end.line - 1] = allLines[range.end.line - 1].replace(/"""/g, "'''");
            }
            return allLines;
        }
    },
